...
public class EnergySyncMessage extends AbstractMessage<EnergySyncMessage> {
	...
	public EnergySyncMessage(EntityPlayerMP player) {
		...
		List<Integer> lis2 = Lists.newArrayList(), lis3 = Lists.newArrayList();
		for (BlockPos p : WorldHelper.getCuboid(new BlockPos(player), 6)) {
			if (EnergyHelper.isEnergyInterface(player.worldObj, p) != null) {
				lis1.add(p);
				lis2.add(EnergyHelper.getEnergy(player.worldObj, p));
				lis3.add(EnergyHelper.getMaxEnergy(player.worldObj, p));
			}
		}
		NBTHelper.setLongList(nbt, "lis1", Utils.getLongList(lis1));
		NBTHelper.setIntList(nbt, "lis2", lis2);
		NBTHelper.setIntList(nbt, "lis3", lis3);
	}

	@Override
	public void handleMessage(EntityPlayer player, NBTTagCompound nbt, Side side) {
		List<BlockPos> lis1 = Utils.getBlockPosList(NBTHelper.getLongList(nbt, "lis1"));
		List<Integer> lis2 = NBTHelper.getIntList(nbt, "lis2"), lis3 = NBTHelper.getIntList(nbt, "lis3");
