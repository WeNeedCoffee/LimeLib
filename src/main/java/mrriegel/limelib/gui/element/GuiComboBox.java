...
public class GuiComboBox extends GuiElement {
	...
	public GuiComboBox(int id, int x, int y, List<String> list, int width,
			int maxEntry) {
		this(id, x, y, list, width, maxEntry, 0);
	}
	...
	public void drawForeground(Minecraft mc, int mouseX, int mouseY) {
		...
		for (int i = 0; i < (Math.min(maxEntry, list.size())); i++) {
			...
			while (mc.fontRendererObj.getStringWidth(t) > width - 6)
				t = t.substring(0, t.length() - 1);
			int in = (i + 1) * 9;
			boolean num = mouseX >= this.x + 2 && mouseY >= this.y + in + 5
					&& mouseX < this.x - 2 + width
					&& mouseY < this.y + 9 + in + 5;
			mc.fontRendererObj.drawString(t, this.x + 3, this.y + 4 + in,
					14737632 - (num ? 0x222200 : 0), true);
			if (num) {
				index = i+page - 1;
				changed = true;
			}
